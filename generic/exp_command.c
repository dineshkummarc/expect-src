 ...
 * ----------------------------------------------------------------------------
 * RCS: @(#) $Id: expCommand.c,v 1.1.2.1.2.5 2002/02/13 02:39:41 davygrvy Exp $
 * ----------------------------------------------------------------------------
 */
...
static Tcl_CmdTraceProc	tcl_tracer;
    ...
    ExpState *esPtr;
{
    ...
    /*
     * Since we're closing the channel, not Tcl, we need to get Tcl's buffers
     * flushed.  Because the channel was nonblocking, EAGAINs could leave
     * things buffered.  They need to be synchronously written now!
     */
    Tcl_Flush(esPtr->channel);

#ifdef __WIN32__
#if 0
    if (esPtr->slave && !esPtr->leaveopen) {
	// This causes the error to be returned from close
	result = Tcl_Close(interp, esPtr->slave);
	esPtr->slave = NULL;
    }
#else
    if (esPtr->tclPid && !esPtr->leaveopen) {
	// This causes the error to be seen only at exp_wait
	(void) TerminateProcess((HANDLE) esPtr->tclPid, CONTROL_C_EXIT);
    }
#endif
#endif
    ...
    Tcl_Interp *interp;
{
    ...
    chan = Exp_CreatePairChannel(interp, Tcl_GetStdChannel(TCL_STDIN), Tcl_GetStdChannel(TCL_STDOUT), "exp_user");
    Tcl_RegisterChannel(interp, chan);
    chan = Exp_CreateExpChannel(interp, chan, 0, EXP_NOPID, &tsdPtr->stdinout);
    tsdPtr->stdinout->keepForever = 1;
    Tcl_RegisterChannel(interp, chan);

    chan = Exp_CreateExpChannel(interp, Tcl_GetStdChannel(TCL_STDERR), 0, EXP_NOPID, &tsdPtr->stderrX);
    tsdPtr->stderrX->keepForever = 1;

    if (exp_dev_tty != NULL) {
	Tcl_Channel devtty;
	devtty = Tcl_GetChannel(interp, "exp_tty", NULL);
	if (devtty) {
	    chan = Exp_CreateExpChannel(interp, devtty, 0, EXP_NOPID, &tsdPtr->devtty);
	    tsdPtr->devtty->keepForever = 1;
	} else {
	    exp_dev_tty = EXP_BAD_FILE;
	    tsdPtr->devtty = NULL;
	}
    }
 ...
 * Exp_SpawnObjCmd --
 *
 *	The generic Tcl command that impliments [spawn].  From here,
 *	we call the platform specific functions that implement the
 *	sub behavior.
 *
 * Results:
 *	TCL_OK or TCL_ERROR
...
{
    static char *options[] = {
	"-nottyinit", "-nottycopy", "-noecho", "-console", "-pty", "-open",
	"-leaveopen", /*"-ignore", "-trap",*/ "-environment", "-directory", NULL
    };
    enum options {
	SPAWN_NOTTYINIT, SPAWN_NOTTYCOPY, SPAWN_NOECHO,	SPAWN_CONSOLE,
	SPAWN_PTY, SPAWN_OPEN, SPAWN_LEAVEOPEN, /*SPAWN_IGNORE, SPAWN_TRAP,*/
	SPAWN_ENV, SPAWN_DIR
    };
    int option, j, done=0, len;
    CONST char *arg;
//    CONST char *text;
//    int sig;
    Tcl_Obj *chanName = NULL, *resultObj;
    Exp_SpawnOptionSet opts;
    Tcl_Channel container, original;
    ExpState *esPtr;
    int pid = 0L;
    Tcl_Pid theUglyHandleHackJob = NULL;
	...
	for (j = 1; (j < objc) && !done; j++) {
	    ...
	    if (*arg == '-') {
		if (Tcl_GetIndexFromObj(interp, objv[j], options, "option", 1,
			&option) != TCL_OK) {
		    goto error;
		}
		...
		case SPAWN_OPEN:
			...
			exp_error(interp,
			    "The -open option requires a channel identifier.");
			goto error;
		    }
		...
		case SPAWN_LEAVEOPEN:
			...
			exp_error(interp,
			    "The -leaveopen option requires a channel identifier.");
			goto error;
		    }
#if 0
		case SPAWN_IGNORE:
		    if (objc > j+1) {
			text = Tcl_GetString(objv[++j]);
			sig = exp_string_to_signal(interp, text);
			if (sig == -1) {
			    exp_error(interp,
				    "usage: -ignore %s: unknown signal name.",
				    text);
			    goto error;
			}
			opts.ignore[sig] = TRUE;
			break;
		    } else {
			exp_error(interp,
				"The -ignore option requires a signal name.");
			goto error;
		    }
...
//		    add code here.
		...
		case SPAWN_ENV:
		    if (objc > j+1) {
			if (Tcl_ListObjLength(interp, objv[j+1], &len)) {
			    goto error;
			}
			if (len % 2) {
			    exp_error(interp,
				"The -environment param must be an even numbered list in the form [array get ...] returns.");
			    goto error;
			}
			Tcl_IncrRefCount(opts.env = Tcl_DuplicateObj(objv[++j]));
			break;
		    } else {
			exp_error(interp,
			    "The -environment option requires a list.");
			goto error;
		    }
		    break;
		...
		case SPAWN_DIR:
		    if (objc > j+1) {
			Tcl_IncrRefCount(opts.dir = Tcl_DuplicateObj(objv[++j]));
			break;
		    } else {
			exp_error(interp,
			    "The -directory option requires a parameter.");
			goto error;
		    }
		    break;
	...
	if (!opts.pty_only && !chanName && (objc == j)) {
	    goto usage;
	}
    } else {
usage:
    ...
    if (!chanName) {
	/* create a new 'spawn' channel. */
	if (!(original = Exp_CreateSpawnChannel(interp, &opts, objc-j,
		&objv[j], &pid, &theUglyHandleHackJob))) {
	    goto error;
	}
    } else {
	int mode;
	...
	if (opts.echo) {
	    expStdoutLogU("spawn", 0);
	    expStdoutLogU(" [open ...]\r\n", 0);
	}

	/* get an existing channel. */
	if (!(original = Tcl_GetChannel(interp, Tcl_GetString(chanName), &mode))) {
	    goto error;
	}
	if (!mode) {
	...
	if (!opts.leaveOpen) {
	    if (!Tcl_IsStandardChannel(original)) {
		/* Remove one ref count. */
		Tcl_DetachChannel(interp, original);
	    } else {
		exp_error(interp, "A standard channel can not be used here");
		goto error;
	    }
	}
    }
    ...
    /*
     * Layer the channel we want to expect upon into this one as a
     * container.  We can't use Tcl_StackChannel as it creates the
     * opposite of a container.
     */
    if (!(container = Exp_CreateExpChannel(interp, original, pid,
	    theUglyHandleHackJob, &esPtr))) {
	Tcl_AppendToObj(resultObj, "Can't create new exp channel.", -1);
	goto error;
    }
    esPtr->leaveopen = opts.leaveOpen;

    /* tell user of new spawn id. */
    Tcl_SetVar(interp, SPAWN_ID_VARNAME, esPtr->name, 0);

    /* we return the pid of the new process in the result, if any. */
    Tcl_SetObjResult(interp, Tcl_NewLongObj(pid));
    return TCL_OK;
    ...
    CONST84 char *argv[];
{
    ...
    Tcl_SetIntObj(resultPtr, esPtr->pid);
    return TCL_OK;
  usage:
    ...
    CONST84 char *argv[];
{
    ...
    resultPtr = Tcl_GetObjResult(interp);
    Tcl_SetIntObj(resultPtr, getpid());
    return(TCL_OK);
}
...
#define send_to_user	((clientData == &sendCD_user) || \
	...
	switch (send_style) {
		...
		rc = 0;
#else
		exp_error(interp,"-break: no such viable abstraction yet for the windows platform");
		return(TCL_ERROR);
#endif
		break;
	}
...
#else
		exp_error(interp,"write(spawn_id=??): %s",Tcl_PosixError(interp));
#endif
		rc = TCL_ERROR;
	    }
	    goto finish;
    ...
    CONST84 char *argv[])
{
    ...
    resultPtr = Tcl_GetObjResult(interp);
    Tcl_SetIntObj(resultPtr, old_loguser);
    ...
    CONST84 char *argv[])
{
    ...
    if (argc == 1) {
	resultPtr = Tcl_GetObjResult(interp);
	Tcl_SetIntObj(resultPtr, exp_tcl_debugger_available);
	return TCL_OK;
    }
    ...
    resultPtr = Tcl_GetObjResult(interp);
    Tcl_SetIntObj(resultPtr, exp_tcl_debugger_was_available);
    return TCL_OK;
 usage:
...
#else
	exp_error(interp,"-slave: no such viable abstraction yet on windows");
	return TCL_ERROR;
#endif
    }
    ...
    CONST84 char *argv[])
{
    ...
    if (argc > 1 && streq(argv[1],"-info")) {
	resultPtr = Tcl_GetObjResult(interp);
	Tcl_SetIntObj(resultPtr, trace_level);
	return TCL_OK;
    }
...
#define NO_CHILD -2
	...
	if (!esPtr->sys_waited) {
		...
		while (1) {
		    ...
		    result = Tcl_WaitPid(esPtr->tclPid,&esPtr->wait,0);
	...
	restart:
		result = Tcl_WaitPid(fp->tclPid,&fp->wait_status,WNOHANG);
		if (result == fp->tclPid) {
		...
		result = (Tcl_Pid) NO_CHILD;	/* no children */
		Tcl_ReapDetachedProcs();
	    }
    ...
    if (result == (Tcl_Pid) -1) {
	objPtr = Tcl_NewObj();
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewIntObj(esPtr->pid));
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewStringObj(spawn_id, -1));
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewIntObj(-1));
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewIntObj(errno));
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewStringObj("POSIX", -1));
	Tcl_ListObjAppendElement(NULL, objPtr,
		Tcl_NewStringObj(Tcl_ErrnoId(), -1));
	Tcl_ListObjAppendElement(NULL, objPtr,
		Tcl_NewStringObj(Tcl_ErrnoMsg(errno), -1));
	Tcl_SetObjResult(interp, objPtr);
	result = TCL_OK;
    } else if (result == (Tcl_Pid) NO_CHILD) {
	...
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewIntObj(esPtr->pid));
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewStringObj(spawn_id, -1));
	Tcl_ListObjAppendElement(NULL, objPtr, Tcl_NewIntObj(0));
	Tcl_ListObjAppendElement(NULL, objPtr,
		Tcl_NewIntObj(WEXITSTATUS(esPtr->wait)));
	if (WIFSIGNALED(esPtr->wait)) {
	    Tcl_ListObjAppendElement(NULL, objPtr,
		    Tcl_NewStringObj("CHILDKILLED", -1));
	    Tcl_ListObjAppendElement(NULL, objPtr,
		    Tcl_NewStringObj(Tcl_SignalId((int)(WTERMSIG(esPtr->wait))), -1));
	    Tcl_ListObjAppendElement(NULL, objPtr,
		    Tcl_NewStringObj(Tcl_SignalMsg((int)(WTERMSIG(esPtr->wait))), -1));
	} else if (WIFSTOPPED(esPtr->wait)) {
	    Tcl_ListObjAppendElement(NULL, objPtr,
		    Tcl_NewStringObj("CHILDSUSP", -1));
	    Tcl_ListObjAppendElement(NULL, objPtr,
		    Tcl_NewStringObj(Tcl_SignalId((int)(WSTOPSIG(esPtr->wait))), -1));
	    Tcl_ListObjAppendElement(NULL, objPtr,
		    Tcl_NewStringObj(Tcl_SignalMsg((int)(WSTOPSIG(esPtr->wait))), -1));
	}
	Tcl_SetObjResult(interp, objPtr);
    }
     ...
     * This can cause writes to the result, so guard against that.
     */
    if (!esPtr->open && esPtr->registered) {
	if (objPtr != NULL) {
	    Tcl_Obj *resultObj;
	    ...
	    Tcl_UnregisterChannel(interp, esPtr->channel);
	    resultObj = Tcl_GetObjResult(interp);
	    Tcl_ListObjAppendElement(NULL, objPtr, resultObj);
	    Tcl_SetObjResult(interp, objPtr);
    ...
    CONST84 char *argv[])
{
    ...
    return(TCL_OK);
#else
    exp_error(interp,"no such feature on Win32");
    return TCL_ERROR;
#endif
}
...
return TCL_ERROR;
    ...
    return TCL_OK;
#else
    exp_error(interp,"no such viable abstraction yet for the windows platform");
    return(TCL_ERROR);
#endif
}
