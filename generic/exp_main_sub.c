...
FILE *exp_cmdfile = 0;
CONST char *exp_cmdfilename = NULL;
...
static Tcl_InterpDeleteProc DeleteCmdInfo;
    ...
    int check_for_nostack;
{
	...
	/* else use what's in interp's result */
    ...
    Tcl_Interp *interp;
{
	...
	if (tcl_major < NEED_TCL_MAJOR || 
	    ...
	    sprintf(buf,
		    ...
		    512, exp_argv0, tcl_major, tcl_minor,
...
#endif
#else
    /* Static build, no export of stub table for now */
    if (Tcl_PkgProvide(interp, "Expect", EXP_VERSION) != TCL_OK) {
	return TCL_ERROR;
    }
#endif
