 ...
 * ----------------------------------------------------------------------------
 * RCS: @(#) $Id: expect.c,v 1.1.2.1.2.3 2002/02/10 12:04:22 davygrvy Exp $
 * ----------------------------------------------------------------------------
 */
...
#   undef DEFAULT_UNREACHABLE
#   define DEFAULT_UNREACHABLE default: return -99;
#endif
...
static char *		exp_indirect_update1 _ANSI_ARGS_((Tcl_Interp *interp,
			    ...
			    struct exp_i *exp_i));
static Tcl_VarTraceProc	exp_indirect_update2; /* 2-part Tcl variable names */
    ...
    int key;
{
	 ...
	 * 0 can actually not mean EOF when we have auto-EOL-translation,
	 * as a single \n following \r earlier can be consumed. -- JH
	 */
	if (cc == 0) {
	    int eof = Tcl_Eof(esPtr->channel);
#ifdef DEBUG
	    expDiagLog("expRead got 0 (EOF? %d)\n", eof);
#endif
	    if (eof) {
		cc = EXP_EOF;
	    }
...
#else
		exp_error(interp,"i_read(spawn_id fd=???): %s",
			Tcl_PosixError(interp));
#endif
    ...
    CONST84 char *argv[];
{
	...
	resultPtr = Tcl_GetObjResult(interp);
	Tcl_SetIntObj(resultPtr, parity);
	return TCL_OK;
    }
