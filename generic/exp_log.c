...
#else
    if (tsdPtr->logChannel && expDevttyIs(esPtr)) {
#endif
...
expStdoutLog TCL_VARARGS_DEF(int,arg1)
{
    ...
    length = vsnprintf(bigbuf, BIGBUFSIZ, fmt, args);
    expDiagWriteBytes(bigbuf, length);
    if (tsdPtr->logAll || (LOGUSER && tsdPtr->logChannel)) {
	Tcl_WriteChars(tsdPtr->logChannel, bigbuf, length);
    }
    if (LOGUSER) {
	fprintf(stdout, "%s", bigbuf); /* avoid %-subs in buf */
    }
    ...
    int force_stdout;	/* override value of logUser */
{
    ...
    if (tsdPtr->logAll || (LOGUSER && tsdPtr->logChannel)) {
	Tcl_WriteChars(tsdPtr->logChannel, buf, length);
    }
    if (LOGUSER) {
#ifdef TCL_UTF_MAX
...
/*VARARGS*/
void
expErrorLog TCL_VARARGS_DEF(CONST char *,arg1)
{
    ThreadSpecificData *tsdPtr = TCL_TSD_INIT(&dataKey);
    int length;
    CONST char *fmt;
    ...
    expDiagWriteChars(bigbuf, length);
    fprintf(stderr, "%s", bigbuf); /* avoid %-subs in buf */
    if (tsdPtr->logChannel) {
	Tcl_WriteChars(tsdPtr->logChannel, bigbuf, length);
    }
    va_end(args);
...
/*VARARGS*/
void
expDiagLog TCL_VARARGS_DEF(CONST char *,arg1)
{
    int length;
    CONST char *fmt;
    va_list args;
...
{
    ThreadSpecificData *tsdPtr = TCL_TSD_INIT(&dataKey);
    Tcl_DString *ds = &tsdPtr->printifyScratch;
    int len;
    char d[7];
    Tcl_UniChar ch;
	...
	if (ch == '\r') {
	    ...
	    Tcl_DStringAppend(ds, (char *) &ch, 1);
	} else {
	    len = sprintf(d, "\\u%04x", ch);
	    Tcl_DStringAppend(ds, d, len);
	}
    }
    ...
    Tcl_Obj *obj;
{
    ...
    if ((!tsdPtr->diagToStderr) && (!tsdPtr->diagChannel)) {
	return NULL;
    }
